...
@Controller
@RequestMapping("/insurance/prod/prodbranch")
public class InsuranceProdProductBranchAction extends BaseActionSupport {
	 
	/**
	 * 序列
	 */
	private static final long serialVersionUID = 6904409492851325136L;
	...
	public String findProductBranchList(Model model, Integer page, ProdProductBranch prodProductBranch, Long productId, Long categoryId, HttpServletRequest req) throws BusinessException {
		...
		try{
			ResultHandleT<List<ProdProductBranch>> resultWrapper = prodProductBranchService.findProdProductBranchList(parameters);
			prodProductBranchList = MiscUtils.autoUnboxing(resultWrapper);
		}catch(Exception e) {
			throw new BusinessException(e.getMessage(), e);
		} finally {
			if(prodProductBranchList == null) {
				prodProductBranchList = new ArrayList<ProdProductBranch>();
			}
		}
		...
		for (ProdProductBranch prodProductBranch2 : prodProductBranchList) {
			...
			ResultHandleT<BizBranch> bizBranchWrapper = branchService.findBranchById(prodProductBranch2.getBranchId());
			BizBranch bizBranch = bizBranchWrapper == null || bizBranchWrapper.isFail() || bizBranchWrapper.getReturnContent() == null ?
					null : bizBranchWrapper.getReturnContent();
			prodProductBranch2.setBizBranch(bizBranch);
		...
		ResultHandleT<List<BizBranch>> bizBranchListWrapper = branchService.findBranchListByCategoryId(categoryId);
		List<BizBranch> branchList = bizBranchListWrapper==null || bizBranchListWrapper.isFail() ? null : bizBranchListWrapper.getReturnContent();
		
		model.addAttribute("branchList", branchList);
		model.addAttribute("productId", productId);
		
		ResultHandleT<BizCategory> bizCategoryWrapper = categoryClientService.findCategoryById(categoryId);
		BizCategory bizCategory = bizCategoryWrapper == null || bizCategoryWrapper.isFail() || bizCategoryWrapper.getReturnContent()== null?
				null : bizCategoryWrapper.getReturnContent();
		model.addAttribute("bizCategory", bizCategory);
		model.addAttribute("prodProductBranchMap", branchIdMap);
		return "/prod/insurance/prodbranch/findProductBranchList";
	}
	...
	public String showUpdateProductBranch(Model model, Long productBranchId, Long branchId) throws Exception {
		...
		ResultHandleT<List<BizBranchProp>> propListWrapper = branchPropService.findBranchPropList(params);
		List<BizBranchProp> branchPropList = propListWrapper == null || propListWrapper.isFail() 
				? null : propListWrapper.getReturnContent(); 
		...
		if(branchPropList!=null){
			for(BizBranchProp branchProp : branchPropList){
				...
				List<ProdProductBranchProp> prodProductBranchPropList =  
						MiscUtils.autoUnboxing(prodProductBranchPropService.findProdProductBranchPropList(params));
		...
		ProdProductBranch prodProductBranch = MiscUtils.autoUnboxing(prodProductBranchService.findProdProductBranchById(productBranchId));
		model.addAttribute("productBranch", prodProductBranch);
		return "/prod/insurance/prodbranch/showUpdateBranch";
	}
	...
	public String showAddProductBranch(Model model, Long productId, Long branchId) throws BusinessException {
		...
		if (branchId != null) {
			...
			List<BizBranchProp> list = MiscUtils.autoUnboxing(branchService.findBranchPropListByBranchId(branchId));
			model.addAttribute("branchPropList", list);
		}
		model.addAttribute("addValueTitle", ProdProductProp.PROP_ADD_VALUE_SPLIT);
		model.addAttribute("productId", productId);
		return "/prod/insurance/prodbranch/showAddProductBranch";
	}
	...
	@ResponseBody
	public Object updateProductBranch(ProdProductBranch prodProductBranch) throws Exception {
		if (log.isDebugEnabled()) {
			log.debug("start method<updateProductBranch>");
		}
		//获取原来的规格
		ProdProductBranch oldProdProductBranch = MiscUtils.autoUnboxing(prodProductBranchService.findProdProductBranchById(prodProductBranch.getProductBranchId()));
		//填充产品规格属性列表
	...
	@ResponseBody
	public Object addProductBranch(ProdProductBranch prodProductBranch) throws Exception {
		if (log.isDebugEnabled()) {
		...
		prodProductBranchService.addProdProductBranch(prodProductBranch);
		//添加操作日志
		try {
			comLogService.insert(COM_LOG_OBJECT_TYPE.PROD_PRODUCT_BRANCH, 
			        productId, prodProductBranch.getProductBranchId(), 
					this.getLoginUser().getUserName(), 
					"添加了产品编号为：【"+productId+"】的产品规格：【"+prodProductBranch.getProductBranchId()+"】", 
					COM_LOG_LOG_TYPE.PROD_PRODUCT_BRANCH_CHANGE.name(), 
					"添加产品规格",null);
		} catch (Exception e) {
			// TODO Auto-generated catch block
...
//				}
	...
	@ResponseBody
	public Object editFlag(Long productBranchId, String cancelFlag) throws Exception {
		if (log.isDebugEnabled()) {
	 ...
	 private String getProductBranchChangeLog(ProdProductBranch oldProductBranch,ProdProductBranch newProductBranch){
		 ...
		 if(null != newProductBranch)
		 {
		     ...
		     if(newProductBranch.getProductBranchPropList() !=null && oldProductBranch.getProductBranchPropList() != null 
				...
				//查询该规格下面所有的产品规格（包括无效的）
				List<BizBranchProp> branchPropList = MiscUtils.autoUnboxing(branchPropService.findBranchPropList(params));
