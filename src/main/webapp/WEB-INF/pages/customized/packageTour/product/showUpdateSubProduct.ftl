...
<form action="/vst_admin/prod/product/addProduct.do" method="post" id="dataForm">
       ...
       <div class="p_box box_info p_line">
        ...
        <div class="p_box box_info clearfix mb20">
            <div class="fl operate"><a class="btn btn_cc1" id="save">保存</a>
...
<script>
	...
	//页面加载完毕判断类型是否可以被操作
	if("${prodProduct.subCategoryId}"== "181" && $('#bu').val() == "DESTINATION_BU" && "${prodProduct.productType}" == "INNERLINE" && "${prodProduct.packageType}"== "LVMAMA"){
	...
	//修改bu后判断类型是否可以被操作
	$("${prodProduct.subCategoryId}"== "181" && '#bu').live('change',function(){
		if($('#bu').val() == "DESTINATION_BU" && "${prodProduct.productType}" == "INNERLINE" && "${prodProduct.packageType}"== "LVMAMA"){
			$("input:checkbox[data=propId_615]").attr("disabled",false);
            $("input:checkbox[data=propId_615]").parent().parent().parent().show();
		}else{
			$("input:checkbox[data=propId_615]").attr("checked",false);
