...
<script>
	...
	function checkAndJump(){
		...
		//打开选择品类
		if(productId==""){
			$("#iframeMain").attr("src","/vst_admin/customized/packageTour/product/showAddProduct.do?categoryId="+categoryId+"&timestamp="+Math.random(10));
		}else{
			$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
		    $("#iframeMain").attr("src","/vst_admin/customized/packageTour/product/showUpdateProduct.do?productId="+productId+"&timestamp="+Math.random(10));
		}
	}
    ...
    $("#routeGroupSuggestion,#traffic,#hotel,#line,#ticket").parent("li").click(function(){
		...
		if($("#productId").val()!=null && $("#productId").val()!=""){
			...
			if(menuId == 'traffic'){
				...
				url = "/vst_admin/customized/packageTour/product/showUpdateProduct.do?productId="+$("#productId").val()+"&categoryId="+$("#categoryId").val()+"&suggestionType=Y"
			}
		 	$("#iframeMain").attr("src",url);
