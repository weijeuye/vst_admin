...
<form method="post" action='/vst_admin/dist/distributor/findDistributorList.do' id="searchForm">
        <table class="s_table">
           <tbody>
                <tr>
...
<script>
	...
	$("#new_button").bind("click",function(){
		dialog("/vst_admin/dist/distributor/showAddDistributor.do", "新增分销商", 800, "auto",function(){
        ...
        var resultCode;
		$.ajax({
			url : "/vst_admin/dist/distributor/addDistributor.do",
			type : "post",
			dataType:"json",
			async: false,
	...
	$("a.editProp").bind("click",function(){
		...
		dialog("/vst_admin/dist/distributor/showUpdateDistributor.do?distributorId="+distributorId, "编辑分销商基本信息", 800, "auto" ,function(){
			if(!$("#dataForm").validate().form()){
			return false;
		}
        var resultCode;
		...
		$.ajax({
			url : "/vst_admin/dist/distributor/updateDistributor.do",
			type : "post",
			dataType:"json",
			async: false,
			data : $(".dialog #dataForm").serialize(),
	...
	$("a.editFlag").bind("click",function(){
	 var distributorId=$(this).attr("data"),
	  ...
	  $.confirm(msg, function () {
		$.ajax({
			url : "/vst_admin/dist/distributor/editFlag.do",
			type : "get",
			async: false,
	...
	$("a.editPro").bind("click",function(){
		var distributorId = $(this).attr("data");
		location.href="/vst_admin/dist/distributorGoods/findDistributorGoodsList.do?distributorId="+distributorId
		}
	);
