...
<form method="post" action='/vst_admin/dist/distributorGoods/findDistributorGoodsList.do' id="searchForm">
        <table class="s_table">
            <tbody>
                <tr>
...
<#if pageParam?? && pageParam.items?? &&  pageParam.items?size &gt; 0>  
	...
	<table class="p_table table_center">
                ...
                <tbody>
					...
					<td>
					<#if distDistributorGoods??&&distDistributorGoods.suppGoods??&&distDistributorGoods.suppGoods.suppSupplier??>
						${distDistributorGoods.suppGoods.suppSupplier.supplierName!''}
					</#if>
					</td>
					...
					distDistributorGoods.productBranch.bizBranch.attachFlag=="Y"&&distDistributorGoods??&&distDistributorGoods.suppGoods??&&distDistributorGoods.suppGoods.cancelFlag=="Y">可售
					<#else>不可售</#if>
					</td>
					<td class="oper">
...
<script>
	...
	$("a.cancelProp").bind("click",function(){
		...
		$.ajax({
			url : "/vst_admin/dist/distributorGoods/editFlag.do",
			type : "get",
			async: false,
