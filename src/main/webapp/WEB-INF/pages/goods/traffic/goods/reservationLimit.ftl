		<#if categoryId?? && categoryId==4>
	     ...
	     <form id="reservationLimitForm">
	     	...
	     	<input type="hidden" name="ennameType" value="TRAV_NUM_NO"/>
	     	<input type="hidden" name="occupType" value="TRAV_NUM_NO"/>
							  	...
							  	<#list travNumTypeList as list>
							  		<#if list.code=='TRAV_NUM_ONE' || list.code=='TRAV_NUM_ALL'>
								  		<input type="radio" name="travNumType" value="${list.code!''}"
											   <#if ((comOrderRequired.travNumType)?? && comOrderRequired.travNumType==list.code)||(!((comOrderRequired.travNumType)??)&&list.code=='TRAV_NUM_ALL')>checked</#if> required/>${list.cnName!''}
