...
<div class="iframe_content mt10">
        ...
        <form method="post" action='/vst_admin/goods/traffic/findSuppGoodsList.do' id="searchForm">
 ...
 <!-- 主要内容显示区域\\ -->        
        ...
        <#if (suppGoods.prodProduct.bizCategory.categoryId==25&&suppGoodsList?? && suppGoodsList?size &gt; 0)||suppGoods.prodProduct.bizCategory.categoryId!=25>
        <div class="p_box box_info">
            <div class="f16 mb10">${testKey?substring(1,testKey?last_index_of("_"))}
            <#if suppGoodsList?? && suppGoodsList?size &gt; 0> 
            <a target="iframeMain" class="f12 fb" href="/vst_admin/lineMultiTraffic/goods/timePrice/showGoodsTimePrice.do?prodProduct.productId=${suppGoods.prodProduct.productId!''}&prodProductBranch.bizBranch.branchId=${testKey?substring(testKey?last_index_of("_")+1,testKey?length)}&suppSupplier.supplierId=${suppGoods.supplierId!''}&suppSupplier.supplierName=${suppGoods.suppSupplier.supplierName!''}&bizCategory.categoryId=${suppGoods.prodProduct.bizCategory.categoryId}&apiFlag=${apiFlag}">[查看/维护时间价格表]</a>
            </#if>
            </div>
...
<script>
		...
		$("#new_button").bind("click",function(){
			...
			if($("#supplierId").val()!=null && $("#supplierId").val()!=""){	
				new xDialog("/vst_admin/goods/traffic/showAddSuppGoods.do",{productId:$("input[name='prodProduct.productId']").val(),supplierId:$("#supplierId").val(),contractName:$("#contractName").val(),contractId:$("#contractId").val(),contentManagerId:$("#contentManagerId").val(),contentManagerName:$("#contentManagerName").val(),categoryId:$("input[name='prodProduct.bizCategory.categoryId']").val(),productBranchId:$("#productBranchId").val()},{title:"新增商品",width:800});
			}else{
				$.alert("请先选择供应商！");
			}	
		});
		...
		new xDialog("/vst_admin/goods/traffic/showUpdateSuppGoods.do",{suppGoodsId:suppGoodsId,productId:$("input[name='prodProduct.productId']").val(),supplierId:$("#supplierId").val(),contractId:$("#contractId").val(),contentManagerId:$("#contentManagerId").val(),categoryId:$("input[name='prodProduct.bizCategory.categoryId']").val(),productBranchId:$("#productBranchId").val()},{title:"编辑商品",width:800});
		});
		
		//设置下单限制
	    $("a.reservationLimit").bind("click",function(){
	        var suppGoodsId = $(this).attr("data");
	        location.href="/vst_admin/goods/traffic/showGoodsReservationLimit.do?suppGoodsId="+suppGoodsId+"&categoryId=25";
	    });
		...
		$("a.cancelProp").bind("click",function(){
			...
			$.ajax({
				url : "/vst_admin/goods/traffic/cancelProduct.do",
				type : "post",
				dataType:"JSON",
