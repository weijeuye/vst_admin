...
<div class="p_box">
<form method="post" action='/vst_admin/goods/timePrice/showGoodsTimePrice.do' id="searchForm">
 ...
 <div class="p_box box_info">
 	<input type="hidden" name="timeFApiPriceList[0].auditPrice" id="auditPrice"/>
 	<input type="hidden"  name="timeFApiPriceList[0].auditSettlementPrice" id="auditSettlementPrice"/>
 	<input type="hidden"   name="timeFApiPriceList[0].childPrice" id="childPrice" />
 	<input type="hidden"   name="timeFApiPriceList[0].childSettlementPrice" id="childSettlementPrice" />
 	<input type="hidden"   name="timeFApiPriceList[0].stock" id="stock" />
 	<div class="price_tab">
		 ...
		 <input type="hidden" name="supplierId" value="<#if suppGoods.suppSupplier??>${suppGoods.suppSupplier.supplierId}</#if>">
			...
			<table   class="e_table form-inline" style="width:900px;" goodsId="">
             ...
             <tbody>
                ...
                <tr>
                    <td width="150" class="e_label"><label class="radio">
                    <input type="radio" class="typeSelect"  name="timeFApiPriceList[0].stockType" checked="checked" value="INQUIRE_NO_STOCK"/>现询-未知库存</label></td>
               		<td width="450"></td>
                </tr>
                <tr>
                    <td class="e_label"><label class="radio" style="padding-right:21px;">
                    	<input type="radio" class="typeSelect"  name="timeFApiPriceList[0].stockType" value="INQUIRE_WITH_STOCK"/>现询库存</label></td>
                	 <td></td>
                </tr>
                <tr  id="withStockTr">
                ...
                <tr id="oversellFlagTr_2">
                    <td class="e_label">是否可超卖：</td>
                	<td>
                	<select class="w10" name="timeFApiPriceList[0].oversellFlag"  disabled=disabled>
	                    	<option value="Y">可超卖</option>
	                    	<option value="N">不可超卖</option>
	                 </select></td>
                </tr>
                <tr>
                    <td class="e_label"><label class="radio">
                    <input type="radio" class="typeSelect"   name="timeFApiPriceList[0].stockType" value="CONTROL"/>确定日库存</label></td>
                	<td></td>
                </tr>
                <tr>
                    <td class="e_label">日库存量：</td>
	...
	$("#timePriceSaveButton").click(function(){
	   ...
	   //添加库存验证
		$(".stock").each(function(){
			...
			if(obj.attr("disabled")=="disabled"){
				...
				obj.rules("add",{required : true, number:true ,min:0,messages : {min:'库存必须大于等于0'}});
			}
		})
	   //添加价格验证
	   $(".adult,.child").each(function(){
        ...
        if(auditSettlementPriceInput.attr("disabled")!="disabled"){
            ...
            //判断销售价和计算价关系
            if(parseFloat($("#auditSettlementPrice").val()) > parseFloat($("#auditPrice").val())){
                if(!confirm("成人销售价小于结算价，是否继续")){
                    return;
                }
            }
        }else{
		...
		if(childSettlementPriceInput.attr("disabled")!="disabled"){
            ...
            if(parseFloat($("#childSettlementPrice").val()) > parseFloat($("#childPrice").val())){
                if(!confirm("儿童销售价小于结算价，是否继续")){
                    return;
                }
            }
		}else {
		...
		//库存
        $("#stock").val($(".stock[disabled!=disabled]").val());
		//把提前预定时间转换为分钟数
		...
		$.ajax({
			url : "/vst_admin/goods/traffic/timePrice/editGoodsFApiTimePrice.do",
		...
		$("#search_button").click(function(){
            ...
            function fillData() {
                var that = this,
                    url = "/vst_admin/goods/traffic/timePrice/findGoodsTimePriceList.do";
                ...
                $.ajax({
                    ...
                    type: "POST",
                    dataType: "JSON",
                    data : {suppGoodsId:good.goodsId,supplierId:good.supplierId,specDate:specDate,apiFlag:good.apiFlag},
         ...
         $("input[type=radio][class=good]").click(function () {
				...
				var apiFlag=$(this).attr("dataFlag");
				good.goodsId = goodsId;
				good.supplierId = supplierId;
				good.apiFlag=apiFlag;
                pandora.calendar({
                    sourceFn: fillData,
                    autoRender: true,
                    frequent: true,
		...
		$(".typeSelect").bind("click",function(){
     		...
     		if((that.attr("checked")=="checked")&&that.val()=="INQUIRE_WITH_STOCK"){
     			$("#withStockTr").find("input[type='text']").removeAttr("disabled");
     			$("#oversellFlagTr_2").find("select[name='timeFApiPriceList[0].oversellFlag']").removeAttr("disabled");
     			$("#oversellFlagTr_3").find("select[name='timeFApiPriceList[0].oversellFlag']").attr("disabled","disabled");
     		}else {
     			$("#withStockTr").find("input[type='text']").attr("disabled","disabled");
     			$("#oversellFlagTr_2").find("select[name='timeFApiPriceList[0].oversellFlag']").attr("disabled","disabled");
     			$("#oversellFlagTr_3").find("select[name='timeFApiPriceList[0].oversellFlag']").removeAttr("disabled");
     		}
     	});
     function isPreSale(obj){
	 var parentDiv = obj.parents("#PreSaletable");
	 var val = obj.val();
	 if(val=='Y'){
	 $("tr[name='isPreSaleTr']").show();
	 }else{
	  $("tr[name='isPreSaleTr']").hide();
	 }
	}
