...
<form method="post" action='/vst_admin/show/playMethod/showSelectPlayMethod.do' id="searchForm">
    <table class="s_table">
        <tbody>
            <tr>
            <input type="hidden" id="productId" value="${productId}" name="productId"/>
            <input type="hidden" value="${playMethodId}" name="playMethodId"/>
           		<td class="s_label">选择类型:
	                <select name="playMethodTypeId" id="playMethodTypeId">
...
$(document).ready(function(){
	var opts= document.getElementById("playMethodTypeId");
	for(var i=0;i<opts.options.length;i++){
	    if("1008"==opts.options[i].value){
	        opts.options[i].style.display = "none";
	    }
	}
});
...
$("#make_sure").bind("click",function(){
   ...
   if(checkboxArray.length==0){
     alert("请至少选择一种玩法！");
	   ...
	   $.ajax({
		  url : "/vst_admin/show/playMethod/saveSelectPlayMethod.do",
		  type : "post",
		  dataType:"JSON",
