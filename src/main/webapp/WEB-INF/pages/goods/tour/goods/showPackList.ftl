...
<body>
<div class="iframe_content mt10" style="width:1000px;height:500px;">
<#assign routeNum = '' />
<#assign stayNum = '' />
<#if maxProdLineRoute ?? >
	<#assign routeNum=maxProdLineRoute.routeNum />
	<#assign stayNum=maxProdLineRoute.stayNum />
</#if>
	            ...
	            <div class="box_content">
	                <table class="e_table form-inline" style="width: 900px" >
		                ...
		                <tr style="background-color:#E4E4E4">
		                	<td class="e_label"   style="font-weight:bold;text-align: left">酒店</td>
		                	<td class="e_label" width="10px" style="font-weight:bold;text-align: left">
		                		<td>
		                		<#list prodProduct.prodLineRouteList as prodLineRou> 
		                		   ${prodLineRou.routeName!''}（${prodLineRou.routeNum!''}天${prodLineRou.stayNum!''}晚 ）；
		                		</#list></td>
		                	</td>
		                	<td class="e_label"   style="text-align: right"><a class="btn btn_cc1" onclick="addGroup('${categoryId}')" selectCategoryId="${categoryId}">插入时间段</a></td>
		                </tr>
	                	</tbody>
	                </table>
	            </div>
	    							...
	    							<#if packGroup?? && packGroup.prodPackageGroupHotel??>
	    								<tr>
						                	...
						                	<td  style="text-align: right">
						                		<a class="btn btn_cc1" onclick="addProductBranch('${packGroup.groupId }')" >新增房型</a>
																...
																<#if detail?? && detail.suppGoods??>
																	<tr>
																		...
																		<td>
																			...
																			<a href="javascript:void(0);" class="showLogDialog" param='objectId=${detail.suppGoods.suppGoodsId!''}&objectType=SUPP_GOODS_GOODS&sysName=VST'">操作日志 </a>
											...
											<#list unPackList as unChangedHotel> 
												<tr>
													...
													<td>
														...
														<a href="javascript:void(0);" class="showLogDialog" param='objectId=${unChangedHotel.suppGoodsId!''}&objectType=SUPP_GOODS_GOODS&sysName=VST'>操作日志 </a>
													</td>
												</tr>
											</#list> </#if>
						                </tbody>
...
function addGroup(categoryId){
	var url = "/vst_admin/productPack/line/showAddGroup.do?groupType=CHANGE"  + "&productId=" + $("#productId").val();
	url += "&selectCategoryId=" + categoryId + "&stayNum=" + ${ stayNum};
...
$("a.editFlag").bind("click",function(){
	 ...
	 $.confirm(msg, function () {
	 	$.get("/vst_admin/packageTour/prod/prodbranch/editSuppGoodsFlag.do?suppGoodsId="+suppGoodsId+"&cancelFlag="+cancelFlag, function(result){
...
function selectPackGroupProductBranch(groupId,selectCategoryId){
	...
	var url = "/vst_admin/productPack/line/showSelectChangedHotelProductList.do?groupType=CHANGE" + "&groupId=" + groupId  
...
function updatePackGroupDetail(productBranchId){
	...
	saveDialog = new xDialog("/vst_admin/packageTour/prod/prodbranch/showAddProductBranchTab.do",
...
function addProductBranch(groupId){
	var branchId = '${branchId!''}'; 
	var productId = '${productId!''}';
	var mainProdBranchId = '${mainProdBranchId!''}';
	saveDialog = new xDialog("/vst_admin/packageTour/prod/prodbranch/showAddProductBranchTab.do",
		{"mainProdBranchId":mainProdBranchId, "branchId":branchId, "productId":productId, "groupId":groupId}, {title:"编辑产品规格",width:1000})
}
