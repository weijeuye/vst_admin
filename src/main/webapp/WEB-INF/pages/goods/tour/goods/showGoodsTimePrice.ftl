...
<div class="p_box">
  ...
  <div class="p_box box_info">
        ...
        <div class="p_box clearfix">
        	...
        	<#if categoryCode=='category_route_group' || categoryCode=='category_route_freedom' || categoryCode=='category_route_local'>
	 		...
	 		<#if adultChildGoods??>
 			<label style="display:inline"><input type="checkbox" class="checkGoods" name="suppGoodsId" value="${adultChildGoods.suppGoodsId}"   data_name="${adultChildGoods.goodsName}" data_price_type="${adultChildGoods.priceType}" />${adultChildGoods.goodsName}[${adultChildGoods.suppGoodsId}]</label>
	 		</#if>
	 		</div>
	 		 </#if>
	 		<#if categoryCode=='category_route_hotelcomb'>
			...
			<#assign additionList = goodsMap['addition'] />
	 		<#list additionList as additionGoods>
	 			<label style="display:inline"  <#if additionGoods.cancelFlag!='Y'>cancelFlag="Y"</#if>  ><input type="checkbox" class="checkGoods" name="suppGoodsId" value="${additionGoods.suppGoodsId}"  data_name="${additionGoods.goodsName}" data_price_type="${additionGoods.priceType}" />${additionGoods.goodsName}[${additionGoods.suppGoodsId}]</label>
	 		</#list>
			</div>
			</#if> 
		    <#if categoryCode=='category_route_group' || categoryCode=='category_route_freedom'>
	 		...
	 		<#list upgradList as upgradGoods>
	 			<label style="display:inline"><input type="checkbox" class="checkGoods" name="suppGoodsId" value="${upgradGoods.suppGoodsId}"  data_name="${upgradGoods.goodsName}" data_price_type="${upgradGoods.priceType}" />${upgradGoods.goodsName}[${upgradGoods.suppGoodsId}]</label>
	 		</#list>
	 		</div>
	 		</#if>
		    <#if categoryCode=='category_route_group' || categoryCode=='category_route_freedom'>
	 		...
	 		<#list changedHotelList as changedHotelGoods>
	 			<label style="display:inline"><input type="checkbox" class="checkGoods" name="suppGoodsId" value="${changedHotelGoods.suppGoodsId}"  data_name="${changedHotelGoods.goodsName}" data_price_type="${changedHotelGoods.priceType}" />${changedHotelGoods.goodsName}[${changedHotelGoods.suppGoodsId}]</label>
	 		</#list>
	 		</div>
	 		</#if>
	 		</form>
     ...
     <div style="margin:-10px 0 0 20px">   
        ...
        <div class="p_box clearfix">
        	<#if categoryCode=='category_route_group' || categoryCode=='category_route_freedom' || categoryCode=='category_route_local'>
		 		<div>成人儿童：
		 		<#assign adultChildGoods = goodsMap['adult_child_diff'] />
	 			<label style="display:inline"><input type="checkbox" class="checkGoods adult_child" name="suppGoodsIdList" value="${adultChildGoods.suppGoodsId}"  data_name="${adultChildGoods.goodsName}" data_price_type="${adultChildGoods.priceType}"/>${adultChildGoods.goodsName}[${adultChildGoods.suppGoodsId}]</label>
		 		</div>
	 		...
	 		<#if categoryCode=='category_route_hotelcomb'>
				<div><a id="showInvalidedGoods" href="javascript:void(0);" style="font-size:12px;"  data="1">显示无效商品</a></div>
		 		<div>套餐：	<label  style="display:inline"><input type="checkbox" class="checkbox_comb" name="combAll">全选</label>
		 		<#assign comboDinnerList = goodsMap['combo_dinner'] />
		 		<#list comboDinnerList as comboDinnerGoods>
		    ...
		    <#if categoryCode=='category_route_group' || categoryCode=='category_route_freedom' || categoryCode=='category_route_local' || categoryCode=='category_route_hotelcomb'>
				...
				<#assign additionList = goodsMap['addition'] />
		 		<#list additionList as additionGoods>
		 			<label style="display:inline" <#if additionGoods.cancelFlag!='Y'>cancelFlag="Y"</#if> ><input type="checkbox" class="checkGoods addition" name="suppGoodsIdList" value="${additionGoods.suppGoodsId}"  data_name="${additionGoods.goodsName}" data_price_type="${additionGoods.priceType}" />${additionGoods.goodsName}[${additionGoods.suppGoodsId}]</label>
		 		</#list>
				</div>
			</#if> 
		    ...
		    <#if categoryCode=='category_route_group' || categoryCode=='category_route_freedom'>
		 		...
		 		<#assign upgradList = goodsMap['upgrad'] />
		 		<#list upgradList as upgradGoods>
		 			<label style="display:inline"><input type="checkbox" class="checkGoods upgrade" name="suppGoodsIdList" value="${upgradGoods.suppGoodsId}"   data_name="${upgradGoods.goodsName}" data_price_type="${upgradGoods.priceType}" />${upgradGoods.goodsName}[${upgradGoods.suppGoodsId}]</label>
		 		</#list>
		 		</div>
	 		</#if>
		    ...
		    <#if categoryCode=='category_route_group' || categoryCode=='category_route_freedom'>
		 		...
		 		<#assign changedHotelList = goodsMap['changed_hotel'] />
		 		<#list changedHotelList as changedHotelGoods>
		 			<label style="display:inline"   ><input type="checkbox" class="checkGoods change_hotel" name="suppGoodsIdList" value="${changedHotelGoods.suppGoodsId}" data_name="${changedHotelGoods.goodsName}" data_price_type="${changedHotelGoods.priceType}"  />${changedHotelGoods.goodsName}[${changedHotelGoods.suppGoodsId}]</label>
		 		</#list>
		 		</div>
	 		</#if>
 		</div>
 ...
 <div class="p_box box_info clearfix mb20">
            <div class="fl operate">
	            ...
	            <a href="javascript:void(0);" style="margin-left:100px;" class="showLogDialog btn btn_cc1" param='objectId=${prodProductId}&objectType=SUPP_GOODS_GOODS&sysName=VST'>操作日志</a>
            </div>
        </div>
 </div>
 </div>
...
<#if categoryCode=='category_route_hotelcomb'>
	$("#showInvalidedGoods,#showInvalidedGoods1").click(function(){
		...
		$("input[type=checkbox][name=combAll]").click(function(){
            ...
            function fillData() {
                var that = this,
                    url = "/vst_admin/tour/goods/timePrice/findGoodsTimePriceList.do";
         ...
         $("a.branch_btn").bind("click",function(i){
				...
				if(branchCode=='adult_child_diff'){
					addDialog = new xDialog("/vst_admin/tour/goods/goods/showUpdateSuppGoods.do?productId="+productId,{}, {title:"修改商品",width:900,height:800,iframe:true})
					return;
				}
				...
				//验证互斥问题
				var isPass = checkGroup(categoryId, productId, branchCode);
				if(!isPass){
					$.alert('升级和可换酒店为互斥模式');
					return;
				}
