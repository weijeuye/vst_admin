    ...
    $("#packageGoodsToBranch").bind("click", function() {
		...
		if(strArray.length <= 0){
    	 ...
    	 else
   		 {
    		 ...
    		 $.ajax({
 				type : "POST",
 				url : "/vst_admin/tour/goods/goods/addGoodsToBranch.do?productBranchId="+productBranchId+"&reId="+reId,
 				dataType : "json",
 				data : {key:jsonText},
...
function queryFirstOpenData(searchFlag)
 {
	 $.ajax({
			type : "POST",
			async: true,
			url : "/vst_admin/tour/goods/goods/showGoodsData.do",
			dataType : "html",
...
function openProduct(productId, categoryId, supplierName,suppGoodsId,supplierId){
	...
	if(groupType == "TICKET"){
		//跳转门票新系统
     	...
     	url = '/vst_admin/ticket/goods/goods/showSuppGoodsList.do?productId='+productId+"&automaticQuery=true"+"&supplierName="+supplierNameValue+"&supplierId="+supplierId+"&categoryId="+categoryId;
	}
     window.open(url);
...
function showGoodsTimePrice(productId, branchId, supplierId,suppGoodsId){
	var url="";
