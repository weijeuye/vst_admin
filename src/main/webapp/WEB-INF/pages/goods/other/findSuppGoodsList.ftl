...
<div class="iframe_content mt10">
        ...
        <form method="post" action='/vst_admin/other/goods/findSuppGoodsList.do' id="searchForm">
 ...
 <!-- 主要内容显示区域\\ -->        
    ...
    <#list suppGoodsMap?keys as testKey>
        ...
        <div class="p_box box_info">
            <div class="f16 mb10">${testKey?substring(1,testKey?last_index_of("_"))}</div>
...
var addSuppGoodsDialog, updateSuppGoodsDialog;
...
$("#new_button").bind("click",function(){
	...
	if($("#supplierId").val()!=null && $("#supplierId").val()!=""){	
		addSuppGoodsDialog = new xDialog("/vst_admin/other/goods/showAddSuppGoods.do",{productId:$("input[name='prodProduct.productId']").val(),supplierId:$("#supplierId").val(),contractName:$("#contractName").val(),
			contractId:$("#contractId").val(),contentManagerId:$("#contentManagerId").val(),contentManagerName:$("#contentManagerName").val(),
			categoryId:$("input[name='prodProduct.bizCategory.categoryId']").val(),productBranchId:$("#productBranchId").val()},{title:"新增商品",width:800});
	}else{
		$.alert("请先选择供应商！");
	}	
});
...
$("a.editProp").bind("click",function(){
	...
	updateSuppGoodsDialog = new xDialog("/vst_admin/other/goods/showUpdateSuppGoods.do",{suppGoodsId:suppGoodsId,productId:$("input[name='prodProduct.productId']").val(),
		supplierId:$("#supplierId").val(),contractId:$("#contractId").val(),contentManagerId:$("#contentManagerId").val(),categoryId:$("input[name='prodProduct.bizCategory.categoryId']").val(),
		productBranchId:$("#productBranchId").val()},{title:"编辑商品" ,width:800});
});
...
$("a.cancelProp").bind("click",function(){
	...
	$.ajax({
		url : "/vst_admin/other/goods/cancelGoods.do",
		type : "post",
		dataType:"JSON",
