...
<#if suppGoodsRefund != null && suppGoodsRefund.latestCancelTime != null>
    ...
    <table class="p_table form-inline">
        <tbody>
            ...
            <tr>
				...
				<td colspan=2 class=" operate mt10">
					<input type="text" id="contractNameUpdate" readonly="readonly" name="suppContract.contractName" <#if suppGoods?? && suppGoods.suppContract?? && suppGoods.suppContract.contractName!="">value="${suppGoods.suppContract.contractName}" </#if> required>
            ...
            <tr>
				...
				<td colspan=2><#if suppGoods?? && suppGoods.cancelFlag=='Y'>是<#else>否</#if>
				...
				<#if goodsTypeList??>	
					<#list goodsTypeList as goodsType>
						...
						<#if goodsType=="NOTICETYPE_DISPLAY" &&  prodProduct.productType != "EXPRESS">
							<tr>
									<td colspan=2>
										<input type="radio" name="goodsType" value="${goodsType.code}" checked = "checked" required >${goodsType.cnName}
       ...
       <tr class="pay">
         ...
         <tr class="prePay" id="deductType_tr">
                 ...
                 <td >
                  <select class="w10" change="deductTypeChange()" id="deductType1" name="deductType">
                    ...
                    <input type="hidden" id="deductValue" name="deductValue" value="${suppGoodsRefund.deductValue}"/>                
                 	<input type="text" name="deductValueInput" id="deductValueInput" value="${suppGoodsRefund.deductValueYuan}"  digits=true min=0/>
...
isView();
<#if suppGoodsRefund??&&"UNRETREATANDCHANGE"==suppGoodsRefund.cancelStrategy> 
...
function onSelectContract2(params){
	if(params!=null){
		$("#contractNameUpdate").text(params.contractName);
		$("#contractIdUpdate").val(params.contractId);
...
$("#save").bind("click",function(){
	...
	$.confirm(msg,function(){
		...
		$.ajax({
			url : "/vst_admin/other/goods/updateSuppGoods.do",
			type : "post",
