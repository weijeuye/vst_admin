	...
	<table class="p_table table_center">
		...
		<tbody>
			...
			<tr>
				...
				<#if isDestinationBU?? && isDestinationBU=='Y'>
					<td>
						<#if hotelGoodsData.aperiodicFlag?? && hotelGoodsData.aperiodicFlag=='Y'>期票<#else>普通票</#if>
					</td>
				</#if>
				...
				<td><a style="cursor: pointer"
					onclick="showGoodsTimePrice('${productId!''}','${productBranchId!''}','${hotelGoodsData.supplierId!''}','${hotelGoodsData.suppGoodsId!''}','${hotelOnLineFlag}')">时间价格表</a>
					<a style="cursor: pointer"
					onclick="openProduct('${productId!''}','${categoryId!''}','${hotelGoodsData.supplierName!''}','${hotelGoodsData.suppGoodsId!''}','${hotelGoodsData.supplierId!''}','${hotelOnLineFlag}')">进入产品</a>
				</td>
			</tr>
			</#list>
		</tbody>
...
<div class="no_data mt20">
	<i class="icon-warn32"></i>
	<#if groupType?? && groupType == 'HOTEL'>>
	您要的商品未找到,您可以在VST->产品管理->标准产品管理中新增一个商品!
	<#else>
	您要的商品未找到,您可以在VST->产品管理->酒店产品查询中新增一个商品!
	</#if>
</div>
</#if> 
</#if>