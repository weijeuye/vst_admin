...
function addRow(groupId, selectType) {
	...
	var vv=$("#tab"+groupId).parent().parent().parent().parent().parent().parent();
	var hei=vv.css("height");
	var newhei=parseInt(hei.substring(0,hei.length-2))+30; 
	vv.css("height",newhei)
}
...
//	console.log($("#suggId").val());
	...
	$.ajax({
		url : "/vst_admin/biz/suggestionDetail/addOrModifyDetail.do",
		type : "post",
		dataType : 'json',
		async: false,
