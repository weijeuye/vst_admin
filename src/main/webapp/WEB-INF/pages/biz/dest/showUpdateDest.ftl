        ...
        <table class="p_table form-inline">
            <tbody>
		        ...
		        <tr>
				    ...
				    <td>
		        	<select name="destMark" id="destMark" required=true> 
		        	<#list destMarkList as destMark>
		        		  <option value="${destMark.code!''}"  <#if dest.destMark == destMark.code>selected</#if>>${destMark.cnName!''}</option>
		        	</#list>		
		        	</select>
		            </td>
		    	</tr>
...
$("#parentName").click(function(){
     initDests();
	destSelectDialog = new xDialog("/vst_admin/biz/dest/selectDestList.do?type=main&parentDestId="+$("#parentId").val()+"&destId=${dest.destId}",{},{title:"选择目的地",iframe:true,width:"1000",height:"600"});
});
...
$("#editButton").bind("click",function(){
	...
	$.ajax({
	url : "/vst_admin/biz/dest/updateDest.do",
	type : "post",
	dataType:"json",
	async: false,
...
$("a.otherParentDests").click(function() {
   if($.trim($("#parentName").val())==""){
    ...
    initDests();
	otherDestSelectDialog = new xDialog("/vst_admin/biz/dest/selectDestList.do?type=other&parentDestId="+$("#parentId").val()+ "&destId=${dest.destId}",{},{title:"选择(次)父级目的地",iframe:true,width:"1000",height:"600"});
});  
