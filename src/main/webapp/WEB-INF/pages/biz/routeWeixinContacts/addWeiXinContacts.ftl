...
<style type="text/css">
     ...
     #mydiv{width: 300px; background-color: #fff; height: 380px;}
     #mydiv ul{position: relative; overflow: hidden; list-style: none; margin: 10px 0 15px 10px;}
     #mydiv li{float: left; background-color: #f6f6f6; width: 215px; height: 215px; position: relative; border: 1px solid #ddd; margin-left: 10px; margin-bottom: 10px; cursor: default;}
...
<div>
	<form method="post" id="addForm" >
		<input type="hidden" id="weiXinBuType" name="weiXinContact.weiXinBuType" value="${weiXinBuType}"/>
		<table class="e_table ">
			<tr >
				<td class="w15" style="text-align:right;">类别：</td>
				<td class="w15 ">
					<#if weiXinBuType =='INNER'><span>国内</span>
					<#elseif weiXinBuType =='FOREIGN'><span>出境</span>
					</#if>
		        </td>
			</tr>
			<tr>
	            ...
	            <td >
	            	<select class="form-control w90" id="categoryId" name="weiXinContact.categoryId">
			...
			<tr id="subcategoryTr" style="display:none">
				...
				<td>
					<div class="inline-block">
	            	 <select class="form-control w120" id="subCategoryId" name="weiXinContact.subCategoryId">
	            	 	<option value="" selected>不限</option>
	    				<#list subCategoryList as bizCategory> 
		    				<#if bizCategory.categoryId !=184>
		                    	<option value=${bizCategory.categoryId!''} >${bizCategory.categoryName!''}</option>
	                    	</#if>
		                </#list>
	                </select>
			...
			<tr>
				...
				<td>
					<input class="w90" type="text" id="contactsAccount" maxlength="100" name="weiXinContact.contactsAccount" ">
		        </td>
			</tr>
			<tr>
				<td class="w15" style="text-align:right; ">微信二维码：</td>
				<td>
					<div id="mydiv" style="width:250px;height:250px;">	      
					<ul>
						<li class="drag-ele"  style="display:none">
							<p class="img-head">			                
					            <span class="photoList">
					            <input type='hidden' id="photoId" name="weiXinContact.photoId" value="">
					            <input type='hidden' id="photoUrl" name="weiXinContact.photoUrl" value="">
					            </span>
					            <i title="删除" id="delete">×</i></p>
				            <img width="200" height="200" title="图片" id="img" />
				        </li>
						<li class="addImg" >
			            	添加图片
			        	</li>
...
var $imageOperate = null;
    ...
    $("#delete").bind("click",function(){
    	...
    	$.confirm("确定从图库删除吗？删除后务必上传新的二维码并保存！！！",function(){
		 	$.ajax({
				url : "/photo-back/photo/photo/delete.do",
				type : 'post',
       ...
       $.ajax({
			...
			success : function(result) {
				 if(result.code=="success"){
					...
					$.alert("保存失败："+result.message,function(){
				 		window.parent.location.href="/vst_admin/biz/weiXinContacts/findWeiXinContacts.do?weiXinBuType="+weiXinBuType;
				 	});
				}
			},
