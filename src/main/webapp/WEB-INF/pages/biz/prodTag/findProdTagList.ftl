...
<script>
   ...
   function createItem(name,value){
	...
	$(function(){
		...
		$('#addProdTag').bind('click',function(){
			...
			if(size<=0){
				...
				var title = '批量添加标签';
				addDialog = new xDialog("/vst_admin/biz/prodTag/showAddProdTag.do?objectType=PROD_PRODUCT",{},{title:title,width:800,heigh:700});				
				return false;
			}				
		});
		...
		$('#delProdTag').bind('click',function(){
			...
			if(size<=0) {
				...
				var title = '批量删除标签';
				delDialog = new xDialog("/vst_admin/biz/prodTag/showDelProdTag.do?objectType=PROD_PRODUCT",{},{title:title,width:800,heigh:700});
				return false;
			}			
		});	
		...
		$('#updateProdTagTime').bind('click',function(){
			var size = $("input[type=checkbox][name=objectIds]:checked").size();
			if(size<=0){
				$.alert('请选择修改标签起止时间的产品');
				return false;
			} else {
				var title = '修改起止时间';			
				addDialog = new xDialog("/vst_admin/biz/prodTag/showUpdateProdTagTime.do?objectType=PROD_PRODUCT",{},{title:title,width:800,heigh:700});
				return false;
			}				
		});
		...
		$('#updateTagTimePosition').bind('click',function(){
			...
			if(size<=0){
				...
				var title = '修改标签';						
				addDialog = new xDialog("/vst_admin/biz/prodTag/showUpdateTagTimePosition.do?objectType=PROD_PRODUCT",{},{title:title,width:800,heigh:700});
				return false;
			}				
		});
		...
		$('#updateProdTagShowPosition').bind('click',function() {
			...
			if(size<=0) {
				...
				var title = '修改标签展示位置';			
				addDialog = new xDialog("/vst_admin/biz/prodTag/showUpdateProdTagDisplay.do?objectType=PROD_PRODUCT",{},{title:title,width:800,heigh:700});
				return false;
			}				
		});
		...
		$('.add').bind('click',function(){
			...
			addDialog = new xDialog("/vst_admin/biz/prodTag/showAddProdTag.do?objectId="+objectId+"&objectType=PROD_PRODUCT",{},{title:title,width:800,heigh:700});
		...
		$('.update').bind('click',function(e) {
			...
			updateDialog = new xDialog("/vst_admin/biz/prodTag/showUpdateProdTag.do?objectType=PROD_PRODUCT&objectId="+ objectId+"&tagGroupId="+$('#tagGroup').val(),{},{title:title,width:900,heigh:700});
			return false;
		});			
	...
	function search(){
		...
		var obj = $('#searchForm');
		obj.attr('action','/vst_admin/biz/prodTag/findProdTagList.do');
		obj.submit();	
	}
