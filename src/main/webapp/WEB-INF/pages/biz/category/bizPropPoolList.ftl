...
<form method="post" action='/vst_admin/biz/category/findBizPropPoolList.do' id="dataForm">
    <table class="s_table">
        <tbody>
        	<tr>
...
$("#addButton").bind("click",function(){
    var url="/vst_admin/biz/category/showBizPropPool.do?operate=add";
	...
	var resultCode;
		$.ajax({
			url : "/vst_admin/biz/category/addBizPropPool.do",
			type : "post",
			async: false,
...
$("a.editProp").bind("click",function(){
    ...
    var url="/vst_admin/biz/category/showBizPropPool.do?operate=update&propId="+propId;
	...
	dialog(url, "修改属性池", 800, "auto",function(){
		...
		$("#propType,#propCode,#inputType,#defaultRadio,#defaultInput,#sourceRadio,#suggestText").attr("disabled",false);
		 var resultCode;
		$.ajax({
			url : "/vst_admin/biz/category/updateBizPropPool.do",
			type : "post",
			async: false,
			data : $(".dialog #dataForm").serialize(),
			dataType:'JSON',
...
function confirmAndRefresh(result){
	if (result.code == "success") {
		pandora.dialog({wrapClass: "dialog-mini", content:result.message, okValue:"确定",ok:function(){
		    var url = "/vst_admin/biz/category/findBizPropPoolList.do";
