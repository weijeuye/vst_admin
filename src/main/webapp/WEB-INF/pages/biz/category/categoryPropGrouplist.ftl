...
$("#addCatePropGroupButton").bind("click",function(){
	...
	var url = "/vst_admin/biz/categoryPropGroup/showAddCategoryPropGroup.do?categoryId="+categoryId;
	dialog(url, "添加新分组", 500, "auto",function(){
		...
		$.confirm("确认添加吗 ？", function () {
			$.ajax({
				url : "/vst_admin/biz/categoryPropGroup/addCategoryPropGroup.do",
				type : "post",
				async: false,
...
$("a.editPropGroupFlag").bind("click",function(){
	 ...
	 var url = "/vst_admin/biz/categoryPropGroup/editFlag.do?groupId="+groupId+"&cancelFlag="+cancelFlag;
...
$("a.deletecatePropGroup").bind("click",function(){
    ...
    $.confirm("删除不能恢复,确认删除吗？", function () {
    	var resultCode; 
		$.ajax({
			url : "/vst_admin/biz/categoryPropGroup/deleteCatePropGroup.do",
			type : "get",
			async: false,
...
$("a.editcatePropGroup").bind("click",function(){
	var groupId=$(this).attr("data");
	var url = "/vst_admin/biz/categoryPropGroup/showAddCategoryPropGroup.do?groupId="+groupId;
	dialog(url, "修改分组", 500, "auto",function(){
		...
		$.confirm("确认修改吗 ？", function () {
			$.ajax({
				url : "/vst_admin/biz/categoryPropGroup/updateCategoryPropGroup.do",
				type : "post",
				async: false,
				data : $(".dialog #dataForm").serialize(),
				dataType:'JSON',
