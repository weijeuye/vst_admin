...
$("#addBranchButton").bind("click",function(){
	...
	var url = "/vst_admin/biz/branch/showAddBranch.do?categoryId="+categoryId;
	dialog(url, "添加新规格", 600, "auto",function(){
	   if(!$("#dataForm").validate().form()){
        ...
        var resultCode;
		$.ajax({
			url : "/vst_admin/biz/branch/addBranch.do",
			type : "post",
			async: false,
...
$("a.editSingleBranch").bind("click",function(){
  ...
  var categoryId= $("input[name='categoryId']").val(); 
  var url = "/vst_admin/biz/branch/showAddBranch.do?branchId="+branchId+"&categoryId=" +categoryId;
  ...
  dd = new xDialog(url,{},{title:"修改规格",width:"600",height:"auto",ok:function(){
		...
		$.confirm("确认修改吗 ？", function () {
			$.ajax({
				url : "/vst_admin/biz/branch/updateBranch.do",
				type : "post",
				async: false,
...
$("a.editBranchFlag").bind("click",function(){
	 ...
	 var url = "/vst_admin/biz/branch/editFlag.do?branchId="+branchId+"&cancelFlag="+cancelFlag+"&newDate"+new Date();
...
$("a.editBranchProp").bind("click",function(){
	...
	var url = "/vst_admin/biz/branchProp/findBranchPropsList.do?categoryId="+categoryId+"&branchId=" +branchId;
