...
<form method="post" action='/vst_admin/biz/dict/findDictDefList.do' id="searchForm">
...
$("a.editDictPropDefList").bind("click",function(){
	...
	var url = "/vst_admin/biz/dictPropDef/findDictPropDefList.do";
...
$(function(){
	...
	$("#new_button").bind("click",function(){
		dialog("/vst_admin/biz/dict/showAddDictDef.do", "新增字典定义", 600, "auto", function(){
			//验证
			if(!$("#dataForm").validate().form()){
				return false;
			}
			$.ajax({
				url : "/vst_admin/biz/dict/addDictDef.do",
				type : "post",
	...
	$("a.editDictDef").bind("click",function(){
	     var dictDefId=$(this).attr("data");
	     var url = "/vst_admin/biz/dict/showAddDictDef.do";
	     updateDialog = new xDialog(url,{"dictDefId":dictDefId}, {title:"修改字典定义",width:600,height:"auto",ok:function(){
			...
			$.confirm("确认修改吗 ？", function () {
				$.ajax({
					url : "/vst_admin/biz/dict/updateDictDef.do",
					type : "post",
					async: false,
	...
	$("a.addDict").bind("click",function(){
	    ...
	    var url = "/vst_admin/biz/dict/showAddDict.do?dictDefId="+dictDefId;
	    dialog(url, "新增字典", 650, "auto", function(){
			...
			if(!$("#dataForm").validate().form()){
			 ...
			 $.ajax({
				url : "/vst_admin/biz/dict/addDict.do",
				type : "post",
				data : $(".dialog #dataForm").serialize(),
				dataType:'JSON',
				success : function(result) {
	...
	$("a.editFlag").bind("click",function(){
		 ...
		 var url = "/vst_admin/biz/dict/editDictDefFlag.do?dictDefId="+dictDefId+"&cancelFlag="+cancelFlag+"&newDate="+new Date();
