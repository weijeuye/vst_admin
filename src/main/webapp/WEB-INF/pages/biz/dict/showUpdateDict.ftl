...
$("#editButton").bind("click",function(){
	...
	$.confirm("确认修改吗 ？", function () {
		...
		$.ajax({
			url : "/vst_admin/biz/dict/updateDict.do",
			type : "post",
			dataType:"json",
			async: false,
			   ...
			   if(result.code=="success"){
				   loading.close();
					...
					if(requestSource == 'dict'){
						updateDialog.close();
						window.location = "/vst_admin/biz/dict/findDictList.do";
					}else if(requestSource == 'shipCompany'){
						updateShipCompanyDialog.close();
						window.location = "/vst_admin/biz/bussinessDict/findBussinessDictList.do?dictCode=${dictCode}";
					}
	   			});
				}else {
					loading.close();
