...
<div class="iframe_content">
    <div class="p_box box_info">
		<form method="post" action='/vst_admin/biz/bizSubject/findBizSubjectList.do' id="searchForm">
	        <table class="s_table">
	            <tbody>
	                <tr>
                        ...
                        <td class="s_label">主题小组：</td>
                        <td class="w18">
                            <select name="subjectGroup" id="subjectGroup">
                                <option value="">全部</option>
								<#list subjectGroups as sg>
									<#if (sg.subjectGroup)??>
                                        <option value="${sg.subjectGroup}" <#if (bizSubject.subjectGroup == sg.subjectGroup)>selected</#if>>
										${sg.subjectGroup}
                                        </option>
									</#if>
								</#list>
	...
	<div align="right">
		<a class="btn btn_cc1" id="batchUpdateBizSubject">批量修改主题状态</a>
		    ...
		    <div class="p_box box_info">
		    	<table class="p_table table_center" style="margin-top: 10px;">
		            ...
		            <tbody>
						...
						<tr>
							<td><input type="checkbox" value="${bizSubject.subjectId}" name="objectIds"/></td>
							<td>${bizSubject.subjectId!''}</td>
							<td>${bizSubject.subjectName!''}</td>
							<td>${bizSubject.pinyin!''}</td>
                            <td>${bizSubject.subjectGroup!''}</td>
							...
							<td class="oper">
		                        <a href="javascript:void(0);" class="editBizSubject" data1="${bizSubject.subjectName!''}" data="${bizSubject.subjectId!''}">修改</a>
		                      	<a href="javascript:void(0);" class="showLogDialog"
								   param='parentId=${bizSubject.subjectId}&parentType=TAG_SUBJECT&sysName=VST'>操作日志</a>
...
<script>
	...
	$(function(){
		...
		$("#showAddBizSubject").bind("click",function(){
			showAddOrUpdate = new xDialog("/vst_admin/biz/bizSubject/showSaveOrUpdateBizSubject.do?",{},{title:"新增主题",width:800,height:500});
			return;
		});	
		...
		$(".editBizSubject").bind("click",function(){
			...
			showAddOrUpdate = new xDialog("/vst_admin/biz/bizSubject/showSaveOrUpdateBizSubject.do?subjectId="+subjectId,{},{title:title,width:800,height:500});
			return;
		});
		...
		// 批量修改主题状态
		$('#batchUpdateBizSubject').bind('click',function(){
			...
			if(size<=0){
				...
				var title = '批量修改主题状态';			
				showAddOrUpdate = new xDialog("/vst_admin/biz/bizSubject/batchUpdateBizSubject.do",{},{title:title,width:500,heigh:400});
				return false;
			}				
		});
		...
		$(".deleteBizSubject").bind("click",function(){
			...
			$.confirm('删除此主题则关联关系会同时删除,确认删除此主题',function(){
				$.ajax({
					url:'/vst_admin/biz/bizSubject/deleteBizSubjectByReId.do',
					type:'POST',
		...
		$(".showProdProduct").bind("click",function(){
			var subjectId = $(this).attr('data');
			showProdProduct = new xDialog("/vst_admin/biz/bizSubject/showProdProduct.do?subjectId="+subjectId,{},{title:"主题管理的产品",iframe:true,width:800});
			return;
		});	
	});
