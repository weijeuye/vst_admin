...
<div class="filter">
    <form class="filter-form clearfix" id="searchSholdReadOnlyForm" method="post" action="/vst_admin/o2o/sholdAssCompany/findReadOnlySholdList.do" >
        ...
        if (window.parent.sholdIds.length > 0) {
            ...
            backstage.confirm({
            	...
            	determineCallback: function(){
					...
					$.ajax({
						url : "/vst_admin/o2o/sholdAssCompany/sholdRel.do",
						type : "post",
						data : data,
						success : function(result) {
							...
							if(result.code == "success"){
								backstage.alert({
					   		  		...
					   		  		callback: function () {
					   		  			$(".JS_btn_save").show();
								    	$("#asso_co_shold", window.parent.parent.document).parent("li").click();
								        window.parent.sholdAssCompanyDialog.destroy();
					   		  		}
					   		  	});
							}else {
								backstage.alert({
