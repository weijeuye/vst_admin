...
<form action="/vst_admin/insurance/prod/product/addProduct.do" method="post" id="dataForm">
       ...
       <div class="p_box box_info p_line">
		...
		<div class="p_box box_info p_line">
			  ...
			  <div class="box_content">
                <table class="e_table form-inline">
                    <tbody>
		        	<#if prodProduct.prodDestReList?? && (prodProduct.prodDestReList?size &gt; 0)>
		                <#list prodProduct.prodDestReList as prodDestRe>
			                <tr <#if prodDestRe_index=0>name='no1'</#if>>
			                	<#if prodDestRe_index=0>
						   			<td name="addspan" rowspan='${prodProduct.prodDestReList?size}' class="e_label">目的地：</td>
						   		</#if>
					            <td>
...
$(function(){
	if($("select[name='prodProductPropList[0].propValue']").val() == '734') {
		$("select[name='prodProductPropList[2].propValue']").attr("disabled",false);
	} else {
		$("select[name='prodProductPropList[2].propValue']").attr("disabled",true);
		$("select[name='prodProductPropList[2].propValue']").find("option[value='']").attr("selected",true);
	}
...
var insureDayInput = $(".INPUT_TYPE_NUMBER").find("input:eq(2)");
	...
	$("#save").bind("click",function(){
		...
		$.confirm(msg,function(){
			...
			$.ajax({
				url : "/vst_admin/insurance/prod/product/updateProduct.do",
				type : "post",
				dataType : 'json',
		...
		$.confirm(msg, function () {
			...
			$.ajax({
				...
				success : function(result) {
					...
					window.location.href="/vst_admin/insurance/prod/prodbranch/findProductBranchList.do?productId="+productId+"&categoryId="+categoryId;
				},
				error : function(result) {
					loading.close();
					$.alert(result.message);
...
function showAddFlagSelect(params,index){
	if(index == 0) {
		if($(params).find("option:selected").val() == '734') {
			$("select[name='prodProductPropList[2].propValue']").attr("disabled",false);
		} else {
		$("select[name='prodProductPropList[2].propValue']").attr("disabled",true);
		$("select[name='prodProductPropList[2].propValue']").find("option[value='']").attr("selected",true);
...
$("#new_button").live("click",function(){
	...
	$tbody.append("<tr><td><input type='text' class='w35' name='dest' id='dest_"+rows+"' readonly = 'readonly' required='true' /><input type='hidden' name='prodDestReList["+rows+"].destId' id='destId"+rows+"'/><input type='hidden' name='prodDestReList["+rows+"].productId' value='${prodProduct.productId}'/>&nbsp;<a class='btn btn_cc1' name='del_button'>删除</a></td></tr>"); 
});
