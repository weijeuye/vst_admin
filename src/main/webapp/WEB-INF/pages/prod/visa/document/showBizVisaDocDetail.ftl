		            ...
		            <table class="p_table form-inline">
			            <tbody>
							...
							<tr>
			                    ...
			                    <td >
			                    	<textarea name="content" class="textWidth" maxlength="4000" style="width: 410px; height: 75px;" ></textarea>
			                    </td>
			                </tr>
			                <tr>
...
<script> 
		...
		$("#save_visa_document_Detail").click(function(){
		  ...
		  if(refreshSensitiveWord($("input[type='text'],textarea"))){
			...
			$.confirm(msg,function(){
				...
				$.ajax({
					url : "/vst_admin/visa/visaDoc/saveBizVisaDocDetail.do",
					type : "post",
					dataType : 'json',
	...
	function showUploadTemplate(detailId){
		...
		new xDialog("/vst_admin/visa/visaDoc/showUploadTemplate.do",
		{"detailId":detailId},
	...
	function delBizVisaDocDetail(detailId){
		 if(confirm('确定要删除此材料明细吗(删除此材料明细将会级联删除相应关联材料)'))
		 	$.ajax({
					url : "/vst_admin/visa/visaDoc/delBizVisaDocDetail.do",
					type : "post",
					dataType : 'json',
	...
	function delBizVisaDetailTemplate(obj,templateId){
		if(confirm('确定要删除此模板附件吗?'))
		 	$.ajax({
					url : "/vst_admin/visa/visaDoc/delBizVisaDetailTemplate.do",
					type : "post",
					dataType : 'json',
