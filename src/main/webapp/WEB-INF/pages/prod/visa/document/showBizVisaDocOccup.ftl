			           	 ...
			           	 <#list bizVisaDocOccupList as bizVisaDocOccup> 
				                <tr>
				                	...
				                	<td >
										...
										<a href="javascript:void(0);" class="showLogDialog ml20"
											param='objectId=${bizVisaDocOccup.occupId}&objectType=VISA_DOCUMNET_DETAIL&sysName=VST'>操作日志</a>
				                	</td>
				                </tr>
			                </#list>
			               </#if>
...
<script> 
	...
	$(document).ready(function(){
		...
		$("#add_visa_document_button").click(function(){
			visaDocumentDialog = 
			new xDialog("/vst_admin/visa/visaDoc/showAddBizVisaDocOccup.do",
			{"docId":${bizVisaDoc.docId}},
			{title:"编辑签证材料所属人群",width:400,height:400});
		});
		
		$("#back_button").click(function(){
			location.href="/vst_admin/visa/visaDoc/findBizVisaDocList.do";
		});
	...
	function showBizVisaDocDetail(docId,occupId){
			visaDocumentDialog = 
			new xDialog("/vst_admin/visa/visaDoc/showBizVisaDocDetail.do",
			{"occupId":occupId,"docId":docId},
	...
	function delBizVisaDocOccup(obj,docId,occupId){
		 if(confirm('确定要删除此材料吗(删除此材料将会级联删除相应材料明细等)'))
		 	$.ajax({
					url : "/vst_admin/visa/visaDoc/delBizVisaDocOccup.do",
					type : "post",
					dataType : 'json',
