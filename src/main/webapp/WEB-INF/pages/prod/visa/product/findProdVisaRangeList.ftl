...
<script>
		...
		if($("#productId").val()!=null && $("#productId").val()!=""){	
			new xDialog("/vst_admin/visa/range/showProdVisaRange.do",{productId:$("input[name='productId']").val()},{title:"新增受理领区",width:1000});
		}else{
			$.alert("请先维护产品！");
		}	
		});
		...
		//修改
		$("a.editProp").click(function(){
		var rangeId = $(this).attr("data");
			new xDialog("/vst_admin/visa/range/showProdVisaRange.do",{productId:$("input[name='productId']").val(),rangeId:rangeId},{title:"编辑受理领区",width:1000});
		});
		...
		$("a.cancelProp").bind("click",function(){
			...
			var rangeId = entity.attr("rangeId");
		 	...
		 	$.confirm(msg, function () {
				$.ajax({
					url : "/vst_admin/visa/range/cancelProdVisaRange.do",
					type : "post",
