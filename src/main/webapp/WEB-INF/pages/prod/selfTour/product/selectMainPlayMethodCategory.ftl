...
<div class="iframe_content">
    <!-- 选择区域开始\\ -->
	 ...
	 <input type="hidden" id="havePlayMethod" value="${havePlayMethod!''}" />
    <!-- //选择区域结束 -->
    <div class="p_box box_info" id="otherPlayMethodCategory">
        <table class="s_table" style="margin-top: 10px;">
	       <tr>
	         <td style="width:150px;text-align:center;">已关联玩法</td>
             <td style="width:1500px;text-align:right;">
                 <a href='javascript:void(0);' class='btn btn_cc1' onclick='newButton()'>新增玩法</a>
             </td>
	       </tr>
	    </table>
	</div>
...
//添加其他玩法
function newButton(){
   var havePlay=$("#havePlayMethod").val();
   if(havePlay=="true"){
   	$.alert("已有玩法，不可再添加");
     return;
   }
   var url = "/vst_admin/connects/prod/playMethod/showSelectPlayMethod.do?productId="+productId+"&categoryId="+categoryId;
 
   selectMethodDialog =  new xDialog(url,{}, {title:"选择其他玩法",iframe:true,height:"550",width:800});
}
function getHavePlay(){
	 $.ajax({
