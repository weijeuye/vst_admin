...
<script>
	...
	$("#select").click(function(){
		...
		productSelectDialog = new xDialog("/vst_admin/selfTour/prodGroup/showSelectReProductList.do",{prodProductId:'${prodProductId}',categoryId:'${categoryId}'},{title:"选择关联产品",width:"1000",height:"350"});
	});
	...
	$(".editProp").click(function(){
		...
		$.confirm("确定取消关联吗?",function(){
			$.ajax({
				url : '/vst_admin/selfTour/prodGroup/deleteProdGroup.do',
	...
	$("select[name=seq]").on("change", function(){
		...
		$.confirm("确定需要设置吗?",function(){
			$.ajax({
				url : '/vst_admin/selfTour/prodGroup/setSeq.do',
