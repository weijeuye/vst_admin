...
<form action="/vst_admin/other/product/addProduct.do" method="post" id="dataForm">
...
jQuery.validator.addMethod("isCharCheck", function(value, element) {
	...
	$("#save").click(function(){
		...
		$.confirm(msg,function(){
		  	...
		  	$.ajax({
				url : "/vst_admin/other/prod/product/addProduct.do",
				type : "post",
				dataType : 'json',
				...
				success : function(result) {
					...
					if(result.code == "success"){
						...
						pandora.dialog({wrapClass: "dialog-mini", content:result.message, okValue:"确定",ok:function(){
							...
							window.location = "/vst_admin/other/prod/prodbranch/findProductBranchList.do?productId="+productId+"&categoryId="+categoryId;
						}});
