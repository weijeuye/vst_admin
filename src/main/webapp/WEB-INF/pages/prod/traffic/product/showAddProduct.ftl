	                 	 ...
	                 	 <td>
	                 	 	<#list productTypeList as list>
                  					...
                  					<#if list.code='INNERLINE'>
                  						<input type="radio" name="productType" value="${(list.code)!''}" checked="true"  required />  ${(list.cnName)!''}
                  					</#if>
               	 				</#list>
            ...
            <div class="box_content">
            	<table class="e_table form-inline">
	                ...
	                <tr>
	                	...
	                	<td>
	                		<select name="toBackType" id="toBackType" required>
	              ...
	              <tr>
						<td class="e_label"><i class="cc1">*</i><#if bizCategory.categoryId!=25>去程</#if>出发城市：</td>
						<td>
	                ...
	                <tr>
						<td class="e_label"><i class="cc1">*</i><#if bizCategory.categoryId!=25>去程</#if>到达城市：</td>
						<td>
	                ...
	                <tr id="backStartDisirictTr">
						<td class="e_label"><i class="cc1">*</i>返程出发城市：</td>
						<td>
							<label><input type="text" class="w15" name="backStartDisirictName" id="backStartDisirict" readonly=readonly required=true >
							<input type="hidden" class="w15" name="backStartDisirict" id="backStartDisirictId" required=true>
							&nbsp;</label>
	                    </td>
	                </tr>
	                <tr id="backEndDisirictTr">
						<td class="e_label"><i class="cc1">*</i>返程到达城市：</td>
						<td>
							<label><input type="text" class="w15" name="backEndDisirictName" id="backEndDisirict" readonly=readonly required=true >
							<input type="hidden" class="w15" name="backEndDisirict" id="backEndDisirictId" required=true>
							&nbsp;</label>
	                    </td>
	                </tr>
                	</tbody>
	        		        ...
	        		        <#list bizCatePropGroupList as bizCateP>
	        		           <#if bizCateP.bizCategoryPropList?? && bizCateP.groupName=='预订须知'>
	        		                <#assign globalIndex=0>
	        		                 <#list bizCateP.bizCategoryPropList as bicProp>
	        		                    <tr>
	        		      	            	<td class="e_label">${(bicProp.propName)!''}：</td><td>
	        		      	            	<textarea id="propList${globalIndex}"  maxlength="1000" style="width:600px; height:100px;"  class="textWidthPro"></textarea>
	        		      	            	<input id="propList${globalIndex}Hidden" name="prodProductPropList[${globalIndex}].propValue" type="hidden" value=""  />
	        		      	            	</td>
	        		      	            </tr>
	        		      	           <input name="prodProductPropList[${globalIndex}].propId" type="hidden" value="${bicProp.propId}"/>
...
<script>
  ...
  $(function() {
         ...
         $(".base_info .textWidthPro").keyup(function(event) {
             var newId=$(this).attr("id")+"Hidden";
             $("#"+newId).val($(this).val());
         });
	...
	$("#save").click(function(){
		...
		$.confirm(msg,function(){
			...
			$.ajax({
				url : "/vst_admin/prod/traffic/addProduct.do",
				type : "post",
				dataType : 'json',
	...
	$(function(){
	     $("#backStartDisirictTr").hide();
	     $("#backEndDisirictTr").hide();
	});
	
	$("#toBackType").change(function(){
	    var value =  $("#toBackType").find("option:selected").val();
	    ...
	    if(categoryId == 21){
	       ...
	       if(value == 'N'){
	           ...
	           $("#backStartDisirictTr").show();
	           $("#backEndDisirictTr").show();
	       }
	    }
	});
