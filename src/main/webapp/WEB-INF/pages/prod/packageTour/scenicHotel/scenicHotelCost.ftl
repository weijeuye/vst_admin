...
.single_textarea {height: 54px;}

textarea {
resize: vertical;
width: 500px; height: 100px; margin-top: 0px; margin-bottom: 0px;
}
div.container {
	overflow:hidden;
	padding-left:20px;
...
.multi_line_extra>span.first_column {
	position:absolute; top:0px; left:0px;
}

.multi_line_extra>span.second_column {
	margin-left:20px;
}
...
.single_line > div.multi_line_field {
	margin-left:70px;
}
...
<div class="container">
	<div style="float:left;width:630px;">
		...
		<div class="single_line">
			...
			<div class="multi_line_field" >
					<input type="hidden" name="scenicHotelCostIncludeVo.hotel.productIds" value="${scenicHotelCostIncludeVo.hotel.productIds!''}"/>
				...
				<div  class="multi_line_extra" >
					<span class="first_column">
					<input type="checkbox"  class="js_switch js_hotel"/>
					<input type="hidden" value ="scenicHotelCostIncludeVo.hotel._default"/>
					</span>
					<span class="second_column">
					<#if scenicHotelCostIncludeVo?? &&scenicHotelCostIncludeVo.hotel??>
		...
		<div  class="single_line">
			...
			<div class="multi_line_field">
					<input type="hidden" name="scenicHotelCostIncludeVo.ticket.productIds" value="${scenicHotelCostIncludeVo.ticket.productIds!''}"/>
				...
				<div class="multi_line_extra" >
					<span class="first_column">
					<input type="checkbox" class="js_switch js_ticket"/>
		...
		<div class="single_line">
			...
			<div class="multi_line_field">
			<textarea  class="single_textarea" name="scenicHotelCostIncludeVo.entertainment" ><#if scenicHotelCostIncludeVo?? && scenicHotelCostIncludeVo.entertainment??>${scenicHotelCostIncludeVo.entertainment?if_exists}</#if></textarea>
			</div>
		</div>
		...
		<div class="single_line">
			...
			<div class="multi_line_field">
			<textarea class="single_textarea" name="scenicHotelCostIncludeVo.meal" ><#if scenicHotelCostIncludeVo?? && scenicHotelCostIncludeVo.meal??>${scenicHotelCostIncludeVo.meal?if_exists}</#if></textarea>
			</div>
		</div>
		<div class="single_line" >
			<div class="multi_line_tips">
			...
			<div class="multi_line_field">
			<textarea class="single_textarea"  name="scenicHotelCostIncludeVo.transport" ><#if scenicHotelCostIncludeVo?? && scenicHotelCostIncludeVo.transport??>${scenicHotelCostIncludeVo.transport?if_exists}</#if></textarea>
			</div>
		</div>
		<div class="single_line">
			<div class="multi_line_tips">
			...
			<div class="multi_line_field">
			<textarea class="single_textarea"  name="scenicHotelCostIncludeVo.buyPresent" ><#if scenicHotelCostIncludeVo?? && scenicHotelCostIncludeVo.buyPresent??>${scenicHotelCostIncludeVo.buyPresent?if_exists}</#if></textarea>
			</div>
		</div>
		...
		<div class="single_line">
			...
			<#else>
			<textarea name="scenicHotelCostIncludeVo.supplement" style="height:25px"></textarea>
			</#if>
			</div>
		</div>
	...
	<div class="pkg_info">
		<!--packaged info-->
		<#if map??>
		<#assign hotel =map['HOTEL']/>
		<div >
			<div>
				<h3>被打包酒店信息</h3>
				<#if scenicHotelCostIncludeVo.state[0] == 1>
				<span class="nova-tip-form" style="padding-top:0px"><span class="nova-icon-xs nova-icon-warning"></span>打包信息有更新
				</span>
				</#if>
			</div>
			<#if hotel?? && hotel?size &gt; 0 >
			<#list hotel as ho>
			<dl>
				<dd>${ho.second.content}</dd>
			</dl>
			</#list>
			<#else>
				暂无可售酒店产品
		...
		<div >
			<div>
			<#assign ticket =map['LINE_TICKET']/>
			<h3>被打包门票信息</h3>
			<#if scenicHotelCostIncludeVo.state[1] == 1>
			<span class="nova-tip-form" style="padding-top:0px"><span class="nova-icon-xs nova-icon-warning"></span>打包信息有更新
			</span>
			</#if>
			</div>
                ...
                $.confirm(message, function () {
                        $.ajax({
                            ...
                            success : function(result) {
                                ...
                                if(window.parent && window.parent.finishedCostEdit) {
									window.parent.finishedCostEdit();
								}
                            },
