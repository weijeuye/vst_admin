        ...
        <div class="aside_box" style="overflow-y: auto;">
            ...
            <ul class="pg_list J_list">   
                ...
                <li class="cc1"><a 	target="iframeMain" href='javascript:void(0);'  id="prodFund1">
                	<#if packageType=='LVMAMA' && (productType=='INNER_BORDER_LINE' || productType=='INNERLINE' || productType=='INNERLONGLINE' || productType=='INNERSHORTLINE')>团期维护<#else>退改规则</#if></a>
                </li>
            </ul>
            <h2 class="pg_line f16">预定条款/提示</h2>
            <ul class="pg_list J_list">           		
...
<script>
    ...
    $("#prodFeature").parent("li").click(function(){
        ...
        if($("#productId").val()!=null && $("#productId").val()!=""){
        	$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
            $("#iframeMain").attr("src","/vst_admin/packageTour/prod/product/showUpdateProductFeature.do?productId="+productId+"&categoryId="+categoryId+"&packageType="+packageType+"&productType="+productType+"&timestamp="+Math.random(10));
        }else {
            $.alert("请先创建产品"); 
            return;
        }
    ...
    $("#routeGroupSuggestion,#traffic,#hotel,#line,#ticket").parent("li").click(function(){
		...
		if($("#productId").val()!=null && $("#productId").val()!=""){
			...
			if(menuId == 'traffic'){
				...
				if($("#modelVersion").val() == 'true' || $("#modelVersionToNew").val()=='true'){
				  url = "/vst_admin/dujia/group/product/showUpdateProductSugg.do?productId="+$("#productId").val()+"&categoryId="+$("#categoryId").val()+"&suggestionType=Y"
				}else{
	...
	function validateRoute(){
	   ...
	   if(auto_pack_traffic=='Y' && isuse_packed_route_details=='Y'){
	        return true;
	    }
		//var categoryId = $('#categoryId').val();
		if(categoryId == 15){
   ...
   //点击产品关联
	$("a[name=prodGroup]").parent("li").click(function(){
		if($("#productId").val()!=null && $("#productId").val()!=""){
			$(".pg_title").html("修改产品"+"&nbsp;&nbsp;&nbsp;&nbsp;"+"产品名称："+$("#productName").val()+"   "+"品类:"+$("#categoryName").val()+"   "+"产品ID："+$("#productId").val());
		 	$("#iframeMain").attr("src","/vst_admin/packageTour/prod/product/findProdGroupList.do?prodProductId="+$("#productId").val()+"&categoryId="+$("#categoryId").val());
