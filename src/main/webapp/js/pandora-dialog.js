(function (global, $, undefined) {
    ...
    Dialog.prototype = {
        ...
        _init: function (config) {
            ...
            //回车关掉弹窗
            if (config.wrapClass == "dialog-confirm") {
                $(window).keydown(function(event) {
                    if (event.keyCode == "13") {
                    	$('.dialog-confirm .btn-ok').click();
                    }
                    if (event.keyCode == "27") {
                    	$('.dialog-confirm .btn-cancel').click();
                    }
                });
            }
            
            //回车关掉弹窗
            if (config.wrapClass == "dialog-mini") {
                $(window).keydown(function(event) {
            		if (event.keyCode == "13" || event.keyCode == "27") {
            			$('.dialog-mini .btn-ok').click();
            		}
                });
            }
            ...
            that.content(config.content, config.iframeHeight);
            that.size(config.width, config.height);

            // hack
            that._zIndex(config.zIndex);
            //that.reset();
        ...
        // 创建iframe
        _createIframe: function (url,iframeHeight) {
            var that = this;
        ...
        _mask: function (config) {
            ...
            if ($overlay.size() === 0) {
                ...
                //重新计算当前窗口的z-index值
                $overlay.css("z-index", config.zIndex - 1);
            }
            this._ismask = true;
        },
        /**
        ...
        _unmask: function () {
            ...
            for (var i in list) {
                ...
                if (list.hasOwnProperty(i)) {
                    ...
                    if(list[i]._ismask === true){
                        count++;
                        //保存当前页面上所有dialog的zindex
                        if (list[i].zIndex !== that.zIndex)
            ...
            if (this._ismask === true && count === 1) {
                ...
                //获取当前页面dialog最大的index值  将遮罩层移动至该dialog下
                var maxIndex = Math.max.apply(Math, indexArray);
                $("[data-mask=overlay]").css("z-index", maxIndex - 1);
            }
         ...
         * 置顶对话框
         */
        _zIndex:function(index){
        	Factory.defaults.zIndex=index;
            index = Factory.defaults.zIndex++;
         ...
         */
        content: function (content, iframeHeight) {
            var rulReg = /^(https?:\/\/|\/|\.\/|\.\.\/)/,
                that = this,
            ...
            if (rulReg.test(content)) {
                ...
                // 调用创建iframe 
                content = this._createIframe(content,iframeHeight);
            }
            ...
            this.wrap.find("div.dialog-inner button:first").delay(100).queue(function(){
        	    this.focus();
        	});
            return this;
        },
        resizeWH : function(){
	   		 var cont =this.wrap.find("div.dialog-content"); 
	   		 this.size(cont.width()+20,cont.height()+80); 
	   	},
        /**
        ...
        close: function () {
            ...
            if (universe !== null) {
                ...
                wrap.find("[data-content=content]").attr("style","").html("");
