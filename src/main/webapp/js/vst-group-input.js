...
(function () {
    ...
    var travelArea = $("#travelArea").val();

    //行程
    var $lineTravel = $(".JS_line_travel");
    ...
    lineTravel.add = function () {
        ...
        if(this.length == 1) {
            this.$tabs.find(".JS_button_disable").hide();
        } else {
        ...
        if (type === "internal") {  //国内模板
            ...
            $innerLine = $template.find(".JS_inner_line_internal").clone();
            ...
            $innerLine = $template.find(".JS_inner_line_abroad").clone();
        }
    ...
    lineTravel.disable = function (pos) {
        ...
        if ($tab.hasClass("disabled")) {
            ...
            $tabDisableButton.html("无效该行程");
            $hidden.val(false);
        } else {
            //否则无效
            $tab.addClass("disabled");
            $tabDisableButton.html("有效该行程");
            $hidden.val(true);
        }
    ...
    //项目推荐
    $document.on("click", ".gi-project-add", function () {

        var $this = $(this);
        var $projectDialog = $templateInner.find(".gi-project-dialog");
        var $parent = $this.parent().parent();
        var $table = $parent.find(".gi-project");
        ...
        $.addDialog({
            ...
            "okCallback": function ($content) {
                ...
                $input.each(function (index) {
                    textArray.push($input.eq(index).val());
                });

                var $tr = $("<tr></tr>");
                var $td = null;
                for (var i = 0; i < textArray.length; i++) {
                    $td = $("<td>" + textArray[i] + "</td>");
                    $tr.append($td);
                }
                $td = $("<td><a href='javascript:' class='JS_del_project'>删除</a></td>");
                $tr.append($td);
    ...
    //删除项目推荐
    $document.on("click", ".JS_del_project", function () {
        var $this = $(this);
    ...
    //购物说明
    $document.on("click", ".gi-shop-add", function () {

        var $this = $(this);
        var $shopDialog = $templateInner.find(".gi-shop-dialog");
        var $parent = $this.parent().parent();
        var $table = $parent.find(".gi-shop");
        var $tbody = $table.find("tbody");
        ...
        $.addDialog({
            ...
            "okCallback": function ($content) {
                ...
                $td = $("<td><a href='javascript:' class='JS_del_shop'>删除</a></td>");
                $tr.append($td);
    ...
    //删除购物说明
    $document.on("click", ".JS_del_shop", function () {
        var $this = $(this);
        var $tr = $this.parents("tr");
        $tr.remove();
    });
