...
K.options = {
	...
	indentChar : '\t',
	cssPath : '',
	cssData : '.ke-content { width: 1080px; }',
	minWidth : 650,
	minHeight : 100,
	minChangeSize : 5,
	items : [
	...
	colorTable : [
		['#333333','#666666', '#999999', '#EE3388','#E53333','#FF9900','#4A90E2']
	],
	colorTable2 : [
	      		['#ffffff','#FFFF99']
	      	],
	fontSizeTable : ['14px', '16px', '18px', '20px','22px','24px'],
	htmlTags : {
...
_extend(KCmd, {
	...
	fontsizedesign : function(val) {
		return this.wrap('<span style="font-size:' + val + ';font-family:Microsoft YaHei;font-weight:bold;"></span>').select();
	},
	maincontentfont : function(val) {
		return this.wrap('<span style="font-size:' + val + ';font-family:Microsoft YaHei;font-weight:normal;"></span>').select();
	},
...
function _getInitHtml(themesPath, bodyClass, cssPath, cssData) {
	var arr = [
		...
		'body, td {font:"sans serif",tahoma,verdana,helvetica;}',
	//	'h1{margin:5px 0;}',
		'body, p, div {word-wrap: break-word;}',
		'p {margin:5px 0;}',
		'.richtext-span{color: #999999;}',
...
_extend(KToolbar, KWidget, {
	init : function(options) {
		...
		function find(target) {
			...
			if (knode.hasClass('ke-toolbar-icon') || knode.hasClass('joyu-toolbar-icon')) {
				return knode.parent();
			}
		}
...
_extend(KDialog, KWidget, {
	init : function(options) {
		...
		var title = options.title,
			body = K(options.body, self.doc, options.mark),
...
function _bindNewlineEvent() {
	...
	var brSkipTagMap = _toMap('h1,h2,h3,h4,h5,h6,pre,li'),
		pSkipTagMap = _toMap('span,p,h1,h2,h3,h4,h5,h6,pre,li,blockquote');
	...
	K(doc).keydown(function(e) {
		...
		if (newlineTag === 'br' && !brSkipTagMap[tagName]) {
			e.preventDefault();
			self.insertHtml('<br />&#8203');
			$(".ke-container").scrollLeft(0);
			return;
		}
	...
	create : function() {
		...
		K.each(self.items, function(i, name) {
			if (name == '|') {
				...
				// 分组名称显示
				htmlList.push('<span class="ke-inline-block joyu-ke-bold" style="font-weight:bold;">' + name.groupTitle + ':</span>');
			} else {
			    if (name == "joyuImage") {
			        htmlList.push('<span class="ke-inline-block ke-outline" data-name="' + name + '" title="' + self.lang(name) + '" unselectable="on">');
			        htmlList.push('<span class="joyu-toolbar-icon ke-inline-block ke-toolbar-icon ke-toolbar-icon-url ke-icon-image" unselectable="on"></span></span>');
			    }
			    else if (name.indexOf("joyu") > -1) {
			        htmlList.push('<span class="ke-inline-block joyu-outline" data-name="' + name + '" title="' + self.lang(name) + '" unselectable="on">');
			        htmlList.push('<span class="joyu-toolbar-icon ke-icon-' + name + '" unselectable="on">' + self.lang(name) + '</span></span>');
				}else{
					htmlList.push('<span class="ke-inline-block ke-outline" data-name="' + name + '" title="' + self.lang(name) + '" unselectable="on">');
					htmlList.push('<span class="joyu-toolbar-icon ke-inline-block ke-toolbar-icon ke-toolbar-icon-url ke-icon-' + name + '" unselectable="on"></span></span>');
				}
			}
		});
...
_plugin('core', function(K) {
	...
	_each('forecolor'.split(','), function(i, name) {
		self.clickToolbar(name, function() {
			self.createMenu({
				name : name,
	...
	self.clickToolbar('hilitecolor', function() {
		self.createMenu({
			name : 'hilitecolor',
			selectedColor : self.cmd.val('hilitecolor') || 'default',
			colors : self.colorTable2,
			click : function(color) {
				self.exec('hilitecolor', color).hideMenu();
			}
		});
	});
	_each(('cut,copy,paste').split(','), function(i, name) {
