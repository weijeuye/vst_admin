    ...
    lvmamaEditor.prototype = {
        initialize: function () {
            ...
            K.plugin("joyuFirstLevelTitle", function (k) {
                ...
                var clickCallBack = function () {
                    editor.exec("fontsizedesign", "24px");
            ...
            K.plugin("joyuSecondLevelTitle", function (k) {
                ...
                var clickCallBack = function () {
                    editor.exec("fontsizedesign", "22px");
                }
                editor.clickToolbar(name, clickCallBack);
            });
            // 三级标题
            K.plugin("joyuThreeLevelTitle", function (k) {
                var editor = this, name = "joyuThreeLevelTitle";
                var clickCallBack = function () {
                    editor.exec("fontsizedesign", "20px");
                }
                editor.clickToolbar(name, clickCallBack);
            });
            // 一级正文
            K.plugin("joyuMainContent1", function (k) {
                var editor = this, name = "joyuMainContent1";
                var clickCallBack = function () {
                    editor.exec("maincontentfont", "18px");
                }
                editor.clickToolbar(name, clickCallBack);
            });
         // 二级正文
            K.plugin("joyuMainContent2", function (k) {
                var editor = this, name = "joyuMainContent2";
                var clickCallBack = function () {
                    editor.exec("maincontentfont", "16px");
                }
                editor.clickToolbar(name, clickCallBack);
            });
         // 三级正文
            K.plugin("joyuMainContent3", function (k) {
                var editor = this, name = "joyuMainContent3";
                var clickCallBack = function () {
                    editor.exec("maincontentfont", "14px");
                }
                editor.clickToolbar(name, clickCallBack);
            });
            // 内容信息
        ...
        editorCreate: function (attrName,attrVal) {
            var editor = K.create('['+attrName+'='+attrVal+']', {
            	...
            	{
                    groupTitle: "标题"
                },"joyuFirstLevelTitle"," ","joyuSecondLevelTitle"," ","joyuThreeLevelTitle"," ",
                { groupTitle: "正文"},
                "joyuMainContent1"," ","joyuMainContent2"," ","joyuMainContent3"," ",
                'forecolor','hilitecolor','bold','lineheight',
                "insertorderedlist",'insertunorderedlist','indent', 'outdent',
                'justifyleft', 'justifycenter', 'justifyright','|',
                'removeformat','clearhtml','source'
                ],
                afterFocus:function () {  //输入文字事件
                    if(window.frames[0].frames.frameElement.contentDocument.body.innerText=="富文本内容为可选项，可根据产品特色添加富文本内容，输入不得大于600px"){
                    	window.frames[0].frames.frameElement.contentDocument.body.innerText="";
                    	window.frames[0].frames.frameElement.contentDocument.body.innerHTML="";
                    }
                },
                afterChange : function () {  //输入文字事件
                    var autoheight =window.frames[0].frames.frameElement.contentDocument.body.scrollHeight;  
                    $(".now_word_surplus").html(autoheight+"/600");
                }
